...
{
	DS18B20_Mode_OUT_PP(Port,Pin);		// 主机输出
	
	//DS18B20_OUT_0;						// 主机至少产生 480us 的低电平复位信号 
	HAL_GPIO_WritePin(Port, Pin, GPIO_PIN_RESET);		// 主机至少产生 480us 的低电平复位信号 
	bsp_delay_us(750);
...
static uint8_t DS18B20_Presence(GPIO_TypeDef * Port,uint16_t Pin)
{
	...
	DS18B20_Mode_IN_NP(Port,Pin);				// 主机设为输入 
	
	// 等待存在脉冲的到来，存在脉冲为一个 60 ~ 240 us 的低电平信号 
	// 如果存在脉冲没有来则做超时处理，从机接收到主机的复位信号后，会在 15 ~ 60 us 后给主机发一个存在脉冲
 
	while (HAL_GPIO_ReadPin(Port, Pin) && (pulse_time < 100))		// 等待低电平响应脉冲
	{
		pulse_time++;
		bsp_delay_us(1);
	}	
...
static uint8_t DS18B20_ReadBit(GPIO_TypeDef * Port,uint16_t Pin)
{
	...
	DS18B20_Mode_OUT_PP(Port,Pin);	// 读 0 和读 1 的时间至少要大于 60 us 
	...
	DS18B20_Mode_IN_NP(Port,Pin);		// 设置成输入，释放总线，由外部上拉电阻将总线拉高 
...
static void DS18B20_SkipRom(GPIO_TypeDef * Port,uint16_t Pin)
{
	...
	DS18B20_WriteByte(Port,Pin,0XCC);		/* 跳过 ROM */
}
...
static void DS18B20_MatchRom(GPIO_TypeDef * Port,uint16_t Pin)
{
	...
	DS18B20_WriteByte(Port, Pin,0X55);		/* 匹配 ROM */
}
...
float DS18B20_GetTemp_SkipRom(GPIO_TypeDef * Port,uint16_t Pin)
{
	...
	DS18B20_WriteByte(Port, Pin,0X44);				/* 开始转换 */
	
	DS18B20_SkipRom(Port, Pin);
  DS18B20_WriteByte(Port, Pin,0XBE);				/* 读温度值 */
...
void DS18B20_ReadId(GPIO_TypeDef * Port,uint16_t Pin,uint8_t *ds18b20_id)
{
	...
	DS18B20_WriteByte(Port, Pin,0x33);       //读取序列号
...
float DS18B20_GetTemp_MatchRom(GPIO_TypeDef * Port,uint16_t Pin,uint8_t * ds18b20_id)
{
	...
	DS18B20_MatchRom(Port, Pin);            	/* 匹配ROM */
